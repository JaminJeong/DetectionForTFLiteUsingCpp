...
namespace ContAI {
    ...
    Image32::Image32(const int width,   const int height,
    ...
    Image32::Image32(const tf_tracking::Size& size, const int channel,
    ...
    Image32::Image32(const int width,
                   const int height,
                   const int channel,
                   const ChannelType channelType,
    ...
    Image32* Image32::Crop(
        ...
        Image32* const cropped_image = new Image32(new_width, new_height,
    ...
    void Image32::Resample(const Image8& original, float inputMean, float inputStd)
    {
        WriteInputTensorFromImageRawData(original, inputMean, inputStd);
        return;
    }
    
   
    void Image32::Flip(const FlipType flip)
    {
    ...
    inline int Image32::GetWidth() const { return width_; }
    inline int Image32::GetHeight() const { return height_; }
    inline int Image32::GetChannel() const { return channel_; }
    inline const ChannelType& Image32::GetChannelType() const { return channelType_; }
    
    const float* Image32::data() const { return image_data_; }
    inline int Image32::stride() const { return stride_; }
    
    // Clears image to a single value.
    inline void Image32::Clear(const float& val) {
        memset(image_data_, val, sizeof(*image_data_) * data_size_);
    }
    
    inline float& Image32::At(int x, int y, int c)
    {
    ...
    inline const float& Image32::At(int x, int y, int c) const
    {
        // prevent out of range
        x = std::max(x, 0);
        x = std::min(x, width_less_one_);
    ...
    std::unique_ptr<Image32> Image32::CopyMemory(
                                               const float* pixels,
                                               const int width,
                                               const int height,
                                               const int channel,
                                               ...
                                               )
    {
        ...
        float* data = new float[imageSize * sizeof(float)];
        if (data == nullptr)
            return nullptr;
        
        memcpy(data, pixels, static_cast<size_t>(imageSize * sizeof(float)));
        std::unique_ptr<Image32> image = std::unique_ptr<Image32>(new Image32(
                                                                           width,
                                                                           height,
                                                                           channel,
                                                                           channelType,
    ...
    void Image32::WriteInputTensorFromImageRawData(const Image8& original, float inputMean, float inputStd){
        ...
        if (channel_type == ChannelType::RGB){
            for (int y = 0; y < wanted_height; ++y) {
                ...
                for (int x = 0; x < wanted_width; ++x) {
                    ...
                    for (int c = 0; c < wanted_channels; ++c) {
                        out_pixel[c] = ((float) in_pixel[c] - inputMean) / inputStd;
                    }
                }
            }
        }
            ...
            for (int y = 0; y < wanted_height; ++y) {
                ...
                for (int x = 0; x < wanted_width; ++x) {
                    ...
                    float* out_pixel = out_row + (x * wanted_channels);
                    for (int c = 0; c < wanted_channels; ++c) {
                        out_pixel[c] = ((float) in_pixel[wanted_channels - c - 1] - inputMean) / inputStd;
                    }
                }
            }
        }
    ...
    inline void Image32::Allocate() {
        image_data_ = new float[data_size_];
    ...
    std::ostream& operator<<(std::ostream& stream, const Image32& image){
