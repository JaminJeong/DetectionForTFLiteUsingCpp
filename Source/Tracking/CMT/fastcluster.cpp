...
/*
 @authors : Nanamare(continuumai)
 @contents : handling some errors (nan_error, fenv_error)
 */
class nan_error{
    ...
    void error_print() {
          std::runtime_error(std::string("Generate Nan Error"));
    }
...
class fenv_error{
    ...
    void error_print() {
        std::runtime_error(std::string("Generate Fenv Error"));
    }
...
#pragma GCC diagnostic pop
    ...
    catch(nan_error& exception) {
        exception.error_print();
    }//end catch
    catch(fenv_error& exception) {
        exception.error_print();
    }
}
