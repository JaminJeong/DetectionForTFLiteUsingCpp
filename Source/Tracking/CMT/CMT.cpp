...
void CMT::initialize(const cv::Mat im_gray, const cv::Rect rect)
{
    ...
    detector->detect(im_gray, _keypoints); // 检测初始图像的所有关键点
    
    printf("Global keypoints size : %d\n", (int)_keypoints.size());
    
    vector<KeyPoint> keypoints;
    
    std::srand(std::time(nullptr));
    
    if(_keypoints.size() > 400) {
        for(int i = 0; i < _keypoints.size() / 2; i++) {
            keypoints.push_back(_keypoints[std::rand() % _keypoints.size()]);
        }
    } else {
        for(int i = 0; i < _keypoints.size(); i++) {
            keypoints.push_back(_keypoints[i]);
        }
    }
    ...
    printf("rectangle range : %d, %d, %d, %d \n", rect.x, rect.y, rect.br().x, rect.br().y);
    ...
    printf("keypoint fore ground size : %d \n", (int)keypoints_fg.size());
    printf("keypoint back ground size : %d \n", (int)keypoints_bg.size());
    ...
    for (size_t i = 0; i < keypoints_fg.size(); i++)
    {
        ...
        classes_active.push_back(classes_fg[i]); // origin classes_active = classes_fg;
    }
    
    //NaNamare Store initial set of keypoints for confidence calculation
...
void CMT::processFrame(cv::Mat im_gray) {
    ...
    float scale = 1;
    float rotation = 0;
//    consensus.estimateScaleRotation(points_fused, classes_fused, scale, rotation); Nanamare
...
//    classes_active = classes_fused;
    ...
    //Nanamare Compute tracking confidence
